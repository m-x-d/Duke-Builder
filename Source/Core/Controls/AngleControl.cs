...
namespace mxd.DukeBuilder.Controls
{
	public partial class AngleControl : UserControl
	{
		...
		public int Value { get { return angle; } set { angle = value; this.Refresh(); } }
		...
		private void AngleSelector_MouseDown(object sender, MouseEventArgs e) 
		{
			...
			{
				angle = thisAngle;
				if(!this.DesignMode && ValueChanged != null) ValueChanged(this, EventArgs.Empty); //Raise event
				this.Refresh();
			}
		}
		...
		private void AngleSelector_MouseMove(object sender, MouseEventArgs e) 
		{
			...
			{
				int newangle = XYToDegrees(new Point(e.X, e.Y), origin);

				if(e.Button == MouseButtons.Left) 
				{
					newangle = (int)Math.Round(newangle / 45f) * 45;
