...
namespace mxd.DukeBuilder.Windows
{
	public interface IMainForm : IWin32Window
	{
		...
		DialogResult ShowEditLines(ICollection<Linedef> lines);
		DialogResult ShowEditWalls(ICollection<Sidedef> walls);
		DialogResult ShowEditSectors(ICollection<Sector> sectors);
		DialogResult ShowEditSprites(ICollection<Thing> things);
		void ShowInfo(MapElement me);
		void HideInfo();
		void RefreshInfo();
		...
		/// <summary>
		/// This browses for an image
		/// </summary>
		/// <returns>Returns the new image index or the index when cancelled</returns>
		int BrowseImage(IWin32Window owner, int initialvalue);

		/// <summary>
		/// THis browses for a thing type
		/// </summary>
		/// <returns>Returns the new thing type or the same thing type when cancelled</returns>
		int BrowseSpriteType(IWin32Window owner, int initialvalue);
