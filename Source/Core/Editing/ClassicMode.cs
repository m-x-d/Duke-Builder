...
namespace mxd.DukeBuilder.Editing
{
	...
	public abstract class ClassicMode : EditMode
	{
		...
		private const float SCALE_MIN = 0.00125f;
		...
		{
			if(General.Settings.AutoScrollSpeed > 0 && !autopanenabled)
			{
				autopanenabled = true;
		...
		public override void OnProcess(double deltatime)
		{
			...
			if(autopanenabled)
			{
				...
				if(panamount.GetManhattanLength() > 0.0f)
				{
					...
					panamount.x = (float)(panamount.x * deltatime);
					panamount.y = (float)(panamount.y * deltatime);
