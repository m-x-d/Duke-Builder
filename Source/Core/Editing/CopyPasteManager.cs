...
namespace mxd.DukeBuilder.Editing
{
	public class CopyPasteManager
	{
		...
		private const string CLIPBOARD_DATA_FORMAT = "DUKE_BUILDER_GEOMETRY";
					...
					if(General.Plugins.OnPasteBegin(options))
					{
						...
						if(General.Editing.Mode.OnPasteBegin(options.Copy()))
						{
							...
							using(Stream memstream = (Stream)Clipboard.GetData(CLIPBOARD_DATA_FORMAT))
							{
								...
								// Read data stream
								General.Map.Map.BeginAddRemove();
								bool success = ClipboardStreamReader.Read(General.Map.Map, memstream);
								General.Map.Map.EndAddRemove();
								if(!success)
								{
		...
		public void CutSelection()
		{
			...
			if(DoCopySelection("Cut selected elements."))
			{
				...
				Actions.Action deleteitem = General.Actions["dukebuilder_deleteitem"];
