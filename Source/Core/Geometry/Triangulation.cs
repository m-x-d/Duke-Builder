	...
	public sealed class Triangulation
	{
		...
		internal void ReadWrite(IReadWriteStream s)
		{
			if(s.IsWriting)
			{
				...
				foreach(int i in islandvertices) s.wInt(i);

				s.wInt(vertices.Count);
				foreach(Vector2D v in vertices) s.wVector2D(v);

				s.wInt(sidedefs.Count);
				foreach(Sidedef side in sidedefs)
				{
					if(side != null)
						s.wInt(side.SerializedIndex);
					else
						s.wInt(-1);
				}
			}
		...
		internal void PostDeserialize(MapSet map)
		{
			...
			foreach(int i in sidedefindices)
			{
				sides.Add(i > -1 ? map.SidedefIndices[i] : null);
			}
		...
		private static void RemoveDoubleSidedefReferences(Dictionary<Sidedef, bool> todosides, ICollection<Sidedef> sides)
		{
			...
			{
				// Double sided and referring to the same sector on both sides?
