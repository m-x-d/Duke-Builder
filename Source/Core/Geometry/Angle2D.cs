...
namespace mxd.DukeBuilder.Geometry
{
	public struct Angle2D
	{
		...
		private const float BUILD_TO_RAD = PI / 2048;
		private const float RAD_TO_BUILD = 2048 / PI;
		...
		// This converts build [0-2047] angle to real angle
		public static float BuildToRad(int buildangle)
		{
			return (float)Math.Round(Normalized((buildangle * BUILD_TO_RAD)), 4);
		}

		// This converts real angle to build angle
		public static int RadToBuild(float realangle)
		{
			return (int)Math.Round(Normalized(realangle) * RAD_TO_BUILD);
		}
