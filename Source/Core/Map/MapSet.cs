...
namespace mxd.DukeBuilder.Map
{
	...
	public sealed class MapSet
	{
		...
		public const string VIRTUAL_SECTOR_FLAG = "!virtual_sector";
		
		// Handler for tag fields
		public delegate void TagHandler<T>(MapElement element, bool actionargument, PropertyType type, ref int value, T obj);
		...
		public int SelectedVerticesCount { get { return sel_vertices.Count; } }
		...
		public ICollection<Vertex> Vertices { get { if(freezearrays == 0) return vertices; return new MapElementCollection<Vertex>(ref vertices, numvertices); } }

		/// <summary>Returns a reference to the list of all linedefs.</summary>
		public ICollection<Linedef> Linedefs { get { if(freezearrays == 0) return linedefs; return new MapElementCollection<Linedef>(ref linedefs, numlinedefs); } }

		/// <summary>Returns a reference to the list of all sidedefs.</summary>
		public ICollection<Sidedef> Sidedefs { get { if(freezearrays == 0) return sidedefs; return new MapElementCollection<Sidedef>(ref sidedefs, numsidedefs); } }

		/// <summary>Returns a reference to the list of all sectors.</summary>
		public ICollection<Sector> Sectors { get { if(freezearrays == 0) return sectors; return new MapElementCollection<Sector>(ref sectors, numsectors); } }

		/// <summary>Returns a reference to the list of all things.</summary>
		public ICollection<Thing> Things { get { if(freezearrays == 0) return things; return new MapElementCollection<Thing>(ref things, numthings); } }
		...
		{
			if(freezearrays == 0) throw new Exception("You must call BeginAddRemove before setting the reserved capacity.");

			if(numvertices < nvertices) Array.Resize(ref vertices, nvertices);
			if(numlinedefs < nlinedefs) Array.Resize(ref linedefs, nlinedefs);
			if(numsidedefs < nsidedefs) Array.Resize(ref sidedefs, nsidedefs);
			if(numsectors < nsectors) Array.Resize(ref sectors, nsectors);
			if(numthings < nthings) Array.Resize(ref things, nthings);
		}
		
		// This ends add/remove operations and crops the arrays
		public void EndAddRemove()
		{
			if(freezearrays > 0) freezearrays--;
			if(freezearrays == 0)
			{
				if(numvertices < vertices.Length) Array.Resize(ref vertices, numvertices);
				if(numlinedefs < linedefs.Length) Array.Resize(ref linedefs, numlinedefs);
				if(numsidedefs < sidedefs.Length) Array.Resize(ref sidedefs, numsidedefs);
				if(numsectors < sectors.Length) Array.Resize(ref sectors, numsectors);
				if(numthings < things.Length) Array.Resize(ref things, numthings);
			}
		}
				...
				if(l.Front != null)
				{
					...
					else
					{
						...
						if(virtualsector == null)
						{
							...
							virtualsector.SetFlag(VIRTUAL_SECTOR_FLAG, true, true);
						}
		...
		{
			if(numsidedefs == General.Map.FormatInterface.MaxWalls)
			{
				General.Interface.DisplayStatus(StatusType.Warning, "Failed to complete operation: maximum number of walls reached.");
				return null;
			}
		...
		public Sector CreateSector()
		{
			...
			// Make the sector
			Sector s = new Sector(this, numsectors);
		...
		{
			if(numthings == General.Map.FormatInterface.MaxSprites)
			{
				General.Interface.DisplayStatus(StatusType.Warning, "Failed to complete operation: maximum number of sprites reached.");
				return null;
			}
		...
		private void RemoveItem<T>(ref T[] array, int index, ref int counter) where T: MapElement
		{
			...
			if(freezearrays == 0) Array.Resize(ref array, counter);
		}
		...
		public int RemoveVirtualSectors()
		{
			...
			{
				// Remove when virtual
				if(sectors[index].IsFlagSet(VIRTUAL_SECTOR_FLAG, true))
				{
		...
		private void NewTagHandler(MapElement element, bool actionargument, PropertyType type, ref int value, Dictionary<int, bool> usedtags)
		{
			usedtags[value] = true;
		}
		...
		{
			//TODO: remove only when hitag/lotag is used as a Channel
			...
			/*foreach(Sector s in sectors)
			{
				...
				{
					int tag = s.Tag;
					handler(s, false, PropertyType.SectorTag, ref tag, obj);
			...
			foreach(Thing t in things)
			{
				...
				{
					int tag = t.Tag;
					handler(t, false, PropertyType.ThingTag, ref tag, obj);
			...
			// Remove tags from walls
			/*foreach(Sidedef s in sidedefs)
			{
				if(s.Marked == marked)
				{
					int tag = s.Tag;
					handler(s, false, PropertyType.LinedefTag, ref tag, obj);
					if(tag != s.Tag) s.Tag = tag;
				}
			}*/
		...
		{
			return (argtype == (int)PropertyType.HiTag) || (argtype == (int)PropertyType.LoTag);
		}
