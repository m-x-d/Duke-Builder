...
{
	//TODO: all things must be inside sectors in Build
	...
	public class CheckStuckThings : ErrorChecker
	{
		...
		public override void Run()
		{
			...
			{
				ThingTypeInfo info = General.Map.Data.GetSpriteInfo(t.Type);
				bool stuck = false;
					...
					foreach(BlockEntry b in blocks)
					{
						...
						if(info.Blocking != ThingTypeInfo.THING_BLOCKING_NONE) {
							foreach(Thing ot in b.Things)
							{
								...
								if(General.Map.Data.GetSpriteInfo(ot.Type).Blocking == ThingTypeInfo.THING_BLOCKING_NONE) continue;
		...
		private bool ThingsOverlap(Thing t1, Thing t2)
		{
			...
			ThingTypeInfo t1info = General.Map.Data.GetSpriteInfo(t1.Type);
			ThingTypeInfo t2info = General.Map.Data.GetSpriteInfo(t2.Type);
