...
{
	[EditMode(DisplayName = "Sprites Mode",
	...
	public class ThingsMode : BaseClassicMode
	{
				...
				if(selected.Count > 0)
				{
					if(General.Interface.IsActiveWindow)
					{
						...
						if(!thinginserted || BuilderPlug.Me.EditNewThing)
						{
							...
							General.Interface.ShowEditSprites(selected);
		...
		public void CopyProperties()
		{
			...
			if(sel != null)
			{
				...
				BuilderPlug.Me.CopiedSpriteProps = new BuildSprite(General.GetFirst(sel));
				General.Interface.DisplayStatus(StatusType.Action, "Copied sprite properties.");
			}
		}
		...
		public void PasteProperties()
		{
			if(BuilderPlug.Me.CopiedSpriteProps != null)
			{
				...
				if(sel != null)
				{
					...
					General.Map.UndoRedo.CreateUndo("Paste sprite properties");
					foreach(Thing t in sel)
					{
						BuilderPlug.Me.CopiedSpriteProps.ApplyTo(t);
						t.UpdateConfiguration();
					}
					General.Interface.DisplayStatus(StatusType.Action, "Pasted sprite properties.");
		...
		public virtual void InsertThing()
		{
			...
			if(mouseinside)
			{
				...
				if(BuilderPlug.Me.EditNewThing)
				{
					// Redraw screen
					General.Interface.RedrawDisplay();
					General.Interface.ShowEditSprites(new List<Thing> {t});
				}
		...
		private static Thing InsertThing(Vector2D pos)
		{
			...
			{
				General.Interface.DisplayStatus(StatusType.Warning, "Failed to insert sprite: outside of map boundaries.");
				return null;
			}
			...
			if(t != null)
			{
				General.Map.Config.ApplyDefaultSpriteSettings(t); //mxd
		...
		public void DeleteItem()
		{
			...
			if(selected.Count > 0)
			{
				...
				if(selected.Count > 1)
				{
					General.Map.UndoRedo.CreateUndo("Delete " + selected.Count + " sprites");
					General.Interface.DisplayStatus(StatusType.Action, "Deleted " + selected.Count + " sprites.");
				}
				else
				{
					General.Map.UndoRedo.CreateUndo("Delete sprite");
					General.Interface.DisplayStatus(StatusType.Action, "Deleted a sprite.");
				}
