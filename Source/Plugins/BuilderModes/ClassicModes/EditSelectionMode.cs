...
namespace mxd.DukeBuilder.EditModes
{
	...
	public class EditSelectionMode : BaseClassicMode
	{
		...
		protected void Highlight(MapElement h)
		{
			...
			if((highlighted != null) && !highlighted.IsDisposed)
			{
				if(highlighted is Vertex)
				{
					if(renderer.StartPlotter(false))
					{
						renderer.PlotVertex((highlighted as Vertex), EditorColor.HIGHLIGHT);
						renderer.Finish();
					}
				}
				else
		...
		private void UpdateGeometry()
		{
			...
			foreach(Thing t in selectedthings)
			{
				t.Angle = Angle2D.Normalized(newthingangle[index++]);
			}
		...
		public override void OnAccept()
		{
			...
			{
				Vector2D tl = new Vector2D(General.Map.FormatInterface.RightBoundary, General.Map.FormatInterface.BottomBoundary);
				Vector2D br = new Vector2D(General.Map.FormatInterface.LeftBoundary, General.Map.FormatInterface.RightBoundary);
				...
				if(tl.x < General.Map.FormatInterface.LeftBoundary || br.x > General.Map.FormatInterface.RightBoundary ||
					tl.y > General.Map.FormatInterface.TopBoundary || br.y < General.Map.FormatInterface.BottomBoundary)
				{
				...
				if(pasting)
				{
					...
					for(int i = 0; i < newsides.Count; i++)
					{
						...
						if(s.Marked && s.Sector.IsFlagSet(MapSet.VIRTUAL_SECTOR_FLAG, true))
						{
							bool joined = false;
					...
					if((vsector != null) && (parent != null))
					{
						...
						if(pasteoptions.AdjustHeights)
						{
							...
							{
								s.CeilingHeight += parent.CeilingHeight - vsector.CeilingHeight;
		...
		public override void OnRedrawDisplay()
		{
			...
			if(renderer.StartPlotter(true))
			{
				...
				if(highlighted is Vertex) renderer.PlotVertex((highlighted as Vertex), EditorColor.HIGHLIGHT);
				renderer.Finish();
			}
