...
namespace mxd.DukeBuilder.EditModes
{
	...
	public sealed class DragThingsMode : BaseClassicMode
	{
		...
		private bool MoveThingsRelative(Vector2D offset, bool snapgrid, bool snapnearest)
		{
			...
			if(offset.x + tl.x < General.Map.FormatInterface.LeftBoundary) offset.x = General.Map.FormatInterface.LeftBoundary - tl.x;
			if(offset.x + br.x > General.Map.FormatInterface.RightBoundary) offset.x = General.Map.FormatInterface.RightBoundary - br.x;
			if(offset.y + tl.y > General.Map.FormatInterface.TopBoundary) offset.y = General.Map.FormatInterface.TopBoundary - tl.y;
			if(offset.y + br.y < General.Map.FormatInterface.BottomBoundary) offset.y = General.Map.FormatInterface.BottomBoundary - br.y;
