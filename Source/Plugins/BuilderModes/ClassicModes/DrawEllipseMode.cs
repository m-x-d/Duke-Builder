...
namespace mxd.DukeBuilder.EditModes
{
	...
	public class DrawEllipseMode : DrawRectangleMode
	{
		...
		override protected Vector2D[] GetShape(Vector2D pStart, Vector2D pEnd) 
		{
			...
			{
				float px = (float)Math.Round(center.x - (float)Math.Sin(curAngle) * hw);
				float py = (float)Math.Round(center.y - (float)Math.Cos(curAngle) * hh);
		...
		[BeginAction("increaseangle")]
		private void IncreaseAngle()
		{
			panel.Angle = General.WrapAngle(panel.Angle + 5);
			angle = Angle2D.DegToRad(panel.Angle);
			Update();
		}

		[BeginAction("decreaseangle")]
