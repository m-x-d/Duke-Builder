...
{
	[EditMode(DisplayName = "Lines Mode",
	...
	public class LinedefsMode : BaseClassicMode
	{
		...
		protected void Highlight(Linedef l)
		{
			...
			//TODO: implement highlight associations
			...
			if(editpressed)
			{
				...
				if(selected.Count > 0)
				{
					if(General.Interface.IsActiveWindow)
					{
						...
						General.Interface.ShowEditLines(selected);
		...
		public void CopyProperties()
		{
			...
			if(sel != null)
			{
				...
				BuilderPlug.Me.CopiedLineProps = new LineProperties(General.GetFirst(sel));
				General.Interface.DisplayStatus(StatusType.Action, "Copied line properties.");
			}
		}
		...
		{
			if(BuilderPlug.Me.CopiedLineProps != null)
			{
				...
				if(sel != null)
				{
					...
					General.Map.UndoRedo.CreateUndo("Paste line properties");
					foreach(Linedef l in sel)
					{
						BuilderPlug.Me.CopiedLineProps.Apply(l);
						l.UpdateCache();
					}
					General.Interface.DisplayStatus(StatusType.Action, "Pasted line properties.");
		...
		public void SelectSingleSided()
		{
			...
			General.Interface.DisplayStatus(StatusType.Action, "Selected only single-sided lines (" + counter + ")");
			General.Interface.RedrawDisplay();
		}
		...
		public void SelectDoubleSided()
		{
			...
			General.Interface.DisplayStatus(StatusType.Action, "Selected only double-sided lines (" + counter + ")");
			General.Interface.RedrawDisplay();
		}
		...
		public void DeleteItem()
		{
			...
			if(selected.Count > 0)
			{
				...
				if(selected.Count > 1)
				{
					General.Map.UndoRedo.CreateUndo("Delete " + selected.Count + " lines");
					General.Interface.DisplayStatus(StatusType.Action, "Deleted " + selected.Count + " lines.");
				}
				else
				{
					General.Map.UndoRedo.CreateUndo("Delete line");
					General.Interface.DisplayStatus(StatusType.Action, "Deleted a line.");
				}
		...
		public void SplitLinedefs()
		{
			...
			if(selected.Count > 0)
			{
				...
				if(selected.Count > 1)
				{
					General.Map.UndoRedo.CreateUndo("Split " + selected.Count + " lines");
					General.Interface.DisplayStatus(StatusType.Action, "Split " + selected.Count + " lines.");
				}
				else
				{
