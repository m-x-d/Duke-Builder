...
namespace mxd.DukeBuilder.EditModes
{
	...
	public class DrawRectangleMode : DrawLinesMode
	{
		#region ================== Variables
				...
				if(points.Count == 1) 
				{
					...
					else if(shape.Length > 3)
					{
						...
						if(alwaysrendershapehints || shape.Length > minpointscount + 1)
						{
							...
							//and shape corners
							for(int i = 0; i < 4; i++)
								renderer.RenderRectangleFilled(new RectangleF(labelCoords[i].x - vsize, labelCoords[i].y - vsize, vsize * 2.0f, vsize * 2.0f), General.Colors.Highlight, true);
						}
					}
					else
					{
						// Render vertex at points[0]
						renderer.RenderRectangleFilled(new RectangleF(start.x - vsize, start.y - vsize, vsize * 2.0f, vsize * 2.0f), General.Colors.Highlight, true);
					}
				} 
				else 
				{
		...
		override public bool DrawPointAt(Vector2D pos, bool stitch, bool stitchline) 
		{
			...
			{ 
				labels.AddRange(new[] { new LineLengthLabel(), new LineLengthLabel(), new LineLengthLabel(), new LineLengthLabel() });
