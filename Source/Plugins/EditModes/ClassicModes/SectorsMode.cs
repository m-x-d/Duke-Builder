...
namespace mxd.DukeBuilder.EditModes
{
	...
	public class SectorsMode : BaseClassicMode
	{
		...
		public override void Dispose()
		{
			...
			{
				// Dispose old labels
				foreach(var l in labels.Values) l.Dispose();
		...
		private static int CreateSelectionCRC(ICollection<Sector> selection)
		{
			...
			foreach(Sector s in selection) crc.Add(s.Index);
		...
		private void SetupLabels()
		{
			...
			{
				// Setup label
				Vector2D v = s.Label.position;
				var label = new TextLabel(20)
				            {
					            TransformCoords = true, 
								Rectangle = new RectangleF(v.x, v.y, 0.0f, 0.0f), 
								AlignX = TextAlignmentX.Center, 
								AlignY = TextAlignmentY.Middle, 
								Scale = 14f, 
								Color = General.Colors.Highlight.WithAlpha(255), 
								Backcolor = General.Colors.Background.WithAlpha(255)
				            };
			...
			if(renderer.StartOverlay(true))
			{
				if(BuilderPlug.Me.ViewSelectionNumbers)
				{
					...
					foreach(Sector s in orderedselection)
					{
						...
						float requiredsize = (labels[s].TextSize.Height / 2) / renderer.Scale;
						if(requiredsize < s.Label.radius) renderer.RenderText(labels[s]);
					}
				}
		...
		private void JoinMergeSectors(bool removelines)
		{
			...
			foreach(Sector s in orderedselection)
			{
				if(s != first && !s.IsDisposed) s.Join(first);
			}
		...
		protected void Highlight(Sector s)
		{
			...
			{
				labels[highlighted].Color = General.Colors.Selection;
			}
			
			// Change label color
			if(s != null && !s.IsDisposed)
			{
				labels[s].Color = General.Colors.Highlight;
			}
			...
			// Show highlight info
			if((highlighted != null) && !highlighted.IsDisposed)
				General.Interface.ShowInfo(highlighted);
			else
		...
		protected void SelectSector(Sector s, bool selectstate, bool update)
		{
			...
			if(!s.IsDisposed)
			{
				...
				if(selectstate && !s.Selected)
				{
					...
					// Setup label
					ICollection<Sector> orderedselection = General.Map.Map.GetSelectedSectors(true);
					labels[s].Text = orderedselection.Count.ToString();
					labels[s].Color = General.Colors.Selection;
				}
				// Deselect the sector?
				...
				if(selectionchanged)
				{
					// Update lines selection
					foreach(Sidedef sd in s.Sidedefs)
					{
						bool front = sd.Line.Front != null && sd.Line.Front.Sector.Selected;
						bool back = sd.Line.Back != null && sd.Line.Back.Sector.Selected;
		...
		private void UpdateSelectedLabels()
		{
			...
			foreach(Sector s in orderedselection)
			{
				...
				labels[s].Text = (index + 1).ToString();
				labels[s].Color = General.Colors.Selection;
		...
		public void CopyProperties()
		{
			...
			if(sel != null)
			{
				...
				BuilderPlug.Me.CopiedSectorProps = new BuildSector(General.GetFirst(sel));
		...
		public void PasteProperties()
		{
			if(BuilderPlug.Me.CopiedSectorProps != null)
			{
				...
				if(sel != null)
				{
					...
					{
						BuilderPlug.Me.CopiedSectorProps.ApplyTo(s);
		...
		public void MakeGradientFloors()
		{
			...
			if(orderedselection.Count > 2)
			{
				float startlevel = General.GetFirst(orderedselection).FloorHeight;
		...
		public void MakeGradientCeilings()
		{
			...
			if(orderedselection.Count > 2)
			{
				float startlevel = General.GetFirst(orderedselection).CeilingHeight;
				float endlevel = General.GetByIndex(orderedselection, orderedselection.Count - 1).CeilingHeight;
				float delta = endlevel - startlevel;
		...
		public void LowerFloors8()
		{
			...
			General.Map.UndoRedo.CreateUndo("Floor heights change", this, (int)UndoGroup.FloorHeightChange, CreateSelectionCRC(selected));
		...
		public void LowerCeilings8()
		{
			...
			General.Map.UndoRedo.CreateUndo("Ceiling heights change", this, (int)UndoGroup.CeilingHeightChange, CreateSelectionCRC(selected));

			// Change heights
			foreach(Sector s in selected) s.CeilingHeight -= 8;
		...
		public void ClearSelection()
		{
			...
			// Clear labels
			foreach(var l in labels.Values) l.Text = "";
